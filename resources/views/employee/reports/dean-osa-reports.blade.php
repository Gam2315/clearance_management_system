...
<div class="page-container">
    ...
    <div class="main-content">
        ...
        <div class="osa-notice">
            <h5><i class="anticon anticon-check-circle"></i> OSA Employee Access</h5>
            <p>You have access to reports as an Office of Student Affairs (OSA) employee.</p>
        </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="table-responsive table-responsive-custom">
                                    <table class="table table-bordered table-white" id="not_cleared_table">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td colspan="8" class="text-center text-muted">Please select an academic year to load reports</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="table-responsive table-responsive-custom">
                                    <table class="table table-bordered table-white" id="cleared_table">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td colspan="6" class="text-center text-muted">Please select an academic year to load reports</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('print_not_cleared').addEventListener('click', function() {
        ...
        const url = `{{ route('employee.reports.print-not-cleared') }}?academic_id=${academicId}&department_id=${departmentId}`;
    ...
    document.getElementById('print_cleared').addEventListener('click', function() {
        ...
        const url = `{{ route('employee.reports.print-cleared') }}?academic_id=${academicId}&department_id=${departmentId}`;
        window.open(url, '_blank');
    });
    ...
    function loadNotClearedStudents(academicId, departmentId) {
        ...
        fetch('{{ route("employee.reports.students-not-cleared") }}', {
            method: 'POST',
            headers: {
                    ...
                    const row = `
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge badge-${student.has_violations ? 'danger' : 'success'}">${student.has_violations ? 'Yes' : 'No'}</span>
                            </td>
                            <td class="text-center">
                                <span class="badge badge-${student.clearance_locked ? 'danger' : 'success'}">${student.clearance_locked ? 'Yes' : 'No'}</span>
                            </td>
    ...
    function loadClearedStudents(academicId, departmentId) {
        ...
        fetch('{{ route("employee.reports.students-cleared") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        .catch(error => {
            console.error('Error loading students cleared:', error);
