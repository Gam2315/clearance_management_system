                        ...
                        <div class="clearance-container">
                            <div class="p-4">
                                ...
                                <div class="form-section">
                                    <form method="POST" action="{{ route('employee.link.nfc') }}" id="student_form">
                                        @csrf
            ...
            function checkNfcCard() {
                ...
                .then(data => {
                    if (data.nfc_detected) {
                        ...
                        // Continue monitoring for card removal - don't clear interval
                    } else {
                        nfcCardDetected = false;

                        // If card was previously detected and now it's not, increment counter
                        if (cardWasDetected) {
                            noCardCount++;

                            // If no card detected for 3 consecutive checks (6 seconds), refresh page
                            if (noCardCount >= 3) {
                                nfcStatus.className = 'nfc-status-card alert alert-info';
                                nfcStatusText.textContent = 'Card removed. Refreshing page...';
                                setTimeout(() => {
                            ...
                            nfcStatusText.textContent = 'Card removed. Please tap your ID again.';
                        } else {
                            nfcStatus.className = 'nfc-status-card alert alert-warning';
